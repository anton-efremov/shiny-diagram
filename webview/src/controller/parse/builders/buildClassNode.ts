...
function parseFieldMember(token: ParseToken, visibility: Visibility, declaration: string): ClassField {
...
function parseMethodMember(token: ParseToken, visibility: Visibility, declaration: string): ClassMethod {
  const methodMatch = /^([^\s(]+)\s*\(([^)]*)\)\s*(.*)$/.exec(declaration);

  if (!methodMatch) {
    return { kind: "method", visibility, name: declaration, params: "", location: toSourceLocation(token) };
  }
