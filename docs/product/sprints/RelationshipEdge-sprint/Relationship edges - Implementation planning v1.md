...
|EditPane / EditPane.tsx _(exists — extend)_|routes selection kind `relationship` to RelationshipEditPane (today it routes to Empty)|+ relationships: full (slices the selected one by relationshipId)|selection: full|+ onRelationshipDuplicate — pass-through|—|—|—|
|EditPane / RelationshipEditPane / RelationshipEditPane.tsx _(new)_|composition of EdgeShapeControls, MultiplicityControls, LabelControls, EdgeActions|relationship: selected (RelationshipView) — pass-through slice|—|onRelationshipDuplicate — pass-through|—|—|—|
|EditPane / RelationshipEditPane / EdgeShapeControls / EdgeShapeControls.tsx|3 dropdowns (source marker: 6 values; line: 2 values; target marker: 6 values) + [Reverse] button|relationship: selected (RelationshipView)|—|—|—|dropdown open (local)|relationship.source.endpointKind.set<br>relationship.target.endpointKind.set<br>relationship.lineKind.set<br>Reverse: see command note below|
|EditPane / RelationshipEditPane / MultiplicityControls / MultiplicityControls.tsx|2 comboboxes: none, `1`, `0..1`, `*`, `0..*`, `1..*`, custom|relationship: selected (RelationshipView)|—|—|—|custom-value drafts (Enter commits)|relationship.source.multiplicity.set<br>relationship.target.multiplicity.set ("none" → null)|
|EditPane / RelationshipEditPane / LabelControls / LabelControls.tsx|label text input + [Remove Label] button|relationship: selected (RelationshipView)|—|—|—|label draft (Enter commits, Esc discards)|relationship.label.set ([Remove Label] → null)|
|EditPane / RelationshipEditPane / EdgeActions / EdgeActions.tsx|[Duplicate] and [Delete] buttons|relationship: selected (RelationshipView) — source of the duplicate seed|—|onRelationshipDuplicate(seed)|—|—|relationship.delete|
