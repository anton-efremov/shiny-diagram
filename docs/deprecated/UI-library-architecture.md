...
| ---------------- | ------------ | --------------------------------------------------------- |
| TextField        | input (nat)  | `value`<br>`disabled`<br>`invalid`<br>`ariaLabel`<br>`autoFocus`<br>`hasEndAction`<br>`appearance` (pane \| inline)<br>`situation` (edgeLabel \| edgeCaption, optional)<br>`onChange`<br>`onBlur`<br>`onKeyDown` |
...
| ReservedBackLink | button (nat) | `label`<br>`visible` (hidden state reserves layout space)<br>`onClick` |
...
| BoxOutline       | —            | `variant` (hover \| selected \| pending)<br>`centerOffset` |
| HaloRing         | —            | `tint`                                                    |
| ResizeAffordance | button (nat) | eight visible squares plus four invisible full-edge hit zones; `centerOffset`; `onGrab(handle, point)` — handle: nw \| n \| ne \| e \| se \| s \| sw \| w |
| Divider          | —            | —                                                         |
...
| Element | Label | Behavior |
| ------- | ----- | -------- |
| ReservedBackLink | "← Back" | restores the originating class selection; hidden state reserves the row so pane content does not shift |
